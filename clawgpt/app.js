...
Example: [0, 2, 5]`;
  ...
  // Extract image data from API response content blocks
  extractImages(content) {
    if (!Array.isArray(content)) return [];
    return content
      .filter(c => c.type === 'image' || c.type === 'image_url' || (c.type === 'image_block'))
      .map(c => {
        // Handle different image formats from APIs
        if (c.source?.type === 'base64' || c.content) {
          const base64Data = c.source?.data || c.content || '';
          const mimeType = c.source?.media_type || c.mimeType || 'image/png';
          return {
            base64: base64Data.startsWith('data:') ? base64Data : `data:${mimeType};base64,${base64Data}`,
            mimeType,
            name: c.name || 'ai-image'
          };
        }
        if (c.image_url?.url) {
          return {
  ...
  selectChat(chatId) {
    ...
    // Pre-load persisted images for this chat into session map
    this.storage.loadChatImages(chatId).then(imageMap => {
      if (imageMap.size > 0) {
        if (!this.sessionImages) this.sessionImages = new Map();
        imageMap.forEach((images, key) => {
          if (!this.sessionImages.has(key)) {
            this.sessionImages.set(key, images);
          }
        });
        // Re-render to show loaded images
        this.renderMessages();
      }
    }).catch(err => console.warn('Failed to load chat images:', err));
  ...
  renderMessages() {
    ...
    this.elements.messages.innerHTML = loadMoreHtml + displayedMessages.map(({ msg, originalIdx }, displayIdx) => {
      ...
      // Check for session images first (current session), then fall back to loading from DB
      ...
      if (sessionImgs && sessionImgs.length > 0) {
        // Have actual images from current session or restored from DB
        imagesHtml = `<div class="message-images">${sessionImgs.map((img, i) => {
          ...
          return `<img src="${safeBase64}" alt="${this.escapeHtml(img.name || 'image')}" title="${this.escapeHtml(img.name || 'image')}" data-img-idx="${i}" data-msg-key="${msgKey}" class="clickable-img" loading="lazy">`;
        }).join('')}</div>`;
      } else if (msg.imageNames && msg.imageNames.length > 0) {
        // Historical message - try loading from IndexedDB, show placeholder with loader
        imagesHtml = `<div class="message-images-placeholder" data-load-images="${msgKey}" data-chat-id="${this.currentChatId}" data-msg-idx="${originalIdx}">${msg.imageNames.map(name =>
          `<span class="image-name"><span class="image-loading-spinner"></span> ${this.escapeHtml(name)}</span>`
        ).join('')}</div>`;
        ...
        imagesHtml = `<div class="message-images">${msg.images.map((img, i) => {
          const safeBase64 = img.base64 && img.base64.startsWith('data:') ? img.base64 : '';
          return `<img src="${safeBase64}" alt="Uploaded image" data-img-idx="${i}" data-msg-key="${msgKey}" class="clickable-img" loading="lazy">`;
        }).join('')}</div>`;
      }
    ...
    // Lazy-load persisted images from IndexedDB for historical messages
    this.loadPersistedImages();

    // Attach image lightbox click handlers
    this.attachImageLightbox();
  ...
  // Lazy-load images from IndexedDB for historical messages
  loadPersistedImages() {
    const placeholders = this.elements.messages.querySelectorAll('[data-load-images]');
    placeholders.forEach(async (el) => {
      const msgKey = el.dataset.loadImages;
      const chatId = el.dataset.chatId;
      const msgIdx = parseInt(el.dataset.msgIdx);

      try {
        const images = await this.storage.loadImages(chatId, msgIdx);
        if (images && images.length > 0) {
          ...
          imagesDiv.innerHTML = images.map((img, i) => {
            const safeBase64 = img.base64 && img.base64.startsWith('data:') ? img.base64 : '';
            return `<img src="${safeBase64}" alt="${this.escapeHtml(img.name || 'image')}" title="${this.escapeHtml(img.name || 'image')}" data-img-idx="${i}" data-msg-key="${msgKey}" class="clickable-img" loading="lazy">`;
          }).join('');
          el.replaceWith(imagesDiv);

          // Attach lightbox to new images
          imagesDiv.querySelectorAll('.clickable-img').forEach(img => {
            img.addEventListener('click', () => this.openImageLightbox(img.src, img.alt));
          });
        } else {
          // No persisted images found - show static placeholder
          el.querySelectorAll('.image-loading-spinner').forEach(s => {
            s.outerHTML = '🖼️';
          });
        }
      } catch (e) {
  ...
  attachImageLightbox() {
    this.elements.messages.querySelectorAll('.clickable-img').forEach(img => {
      img.addEventListener('click', () => this.openImageLightbox(img.src, img.alt));
    });
  }
  ...
  openImageLightbox(src, alt) {
    if (!src) return;
    // Only allow data: URLs and blob: URLs for safety
    ...
    // Build DOM safely to avoid injection
    ...
    downloadLink.download = (alt || 'image') + '.png';
    downloadLink.title = 'Download';
    downloadLink.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>`;
    ...
    // Close on backdrop click, close button, or image click
  ...
  initImageUpload() {
    ...
    if (dropZone) {
      ...
      overlay.innerHTML = `<div class="drag-overlay-text">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  async sendMessage(textOverride = null) {
    ...
    // Keep images in session memory AND persist to IndexedDB
    if (images && images.length > 0) {
      if (!this.sessionImages) this.sessionImages = new Map();
      ...
      this.storage.saveImages(this.currentChatId, this.chats[this.currentChatId].messages.length, images)
        .catch(err => console.warn('Failed to persist user images:', err));
    }
  ...
  handleChatEvent(payload) {
    ...
    const responseImages = this.extractImages(payload.message?.content);
    ...
    if (state === 'delta' && content) {
      ...
      // Accumulate streamed images
      if (responseImages.length > 0) {
        if (!this._streamImages) this._streamImages = [];
        responseImages.forEach(img => {
          if (!this._streamImages.some(existing => existing.base64 === img.base64)) {
            this._streamImages.push(img);
          }
        });
      }
      ...
      // Collect all images from final response + any accumulated during streaming
      const finalImages = [
        ...(this._streamImages || []),
        ...responseImages
      ].filter((img, idx, arr) => arr.findIndex(i => i.base64 === img.base64) === idx);
      this._streamImages = null;

      console.log('Final content length:', finalContent?.length, 'images:', finalImages.length, 'content:', content?.substring(0, 100), 'buffer:', this.streamBuffer?.substring(0, 100));

      if (state === 'error') {
        this.addAssistantMessage('Error: ' + (payload.errorMessage || 'Unknown error'));
      } else if (finalContent || finalImages.length > 0) {
        // Track output tokens
        this.addTokens(this.estimateTokens(finalContent || ''));
        this.addAssistantMessage(finalContent || '', finalImages.length > 0 ? finalImages : null);
      }
  ...
  addAssistantMessage(content, images = null) {
    if (!this.currentChatId || !this.chats[this.currentChatId]) return;
    if ((!content || !content.trim()) && (!images || images.length === 0)) return; // Skip empty messages
    ...
    // Store image names in the message for reference (not full data)
    if (images && images.length > 0) {
      assistantMsg.imageNames = images.map(img => img.name || 'ai-image');
    }

    const msgIndex = this.chats[this.currentChatId].messages.length;
    ...
    // Persist images to IndexedDB and session map
    if (images && images.length > 0) {
      if (!this.sessionImages) this.sessionImages = new Map();
      const msgKey = `${this.currentChatId}-${msgIndex}`;
      this.sessionImages.set(msgKey, images);
      // Persist to IndexedDB for cross-session survival
      this.storage.saveImages(this.currentChatId, msgIndex, images)
        .catch(err => console.warn('Failed to persist AI images:', err));
    }
